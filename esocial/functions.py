...
def create_folders():
    ...
    for service in services:
        lista_pastas = [
            '%s%sComunicacao/%s/header/' % (BASE_DIR,
                                            config.FILES_PATH, service),
            '%s%sComunicacao/%s/request/' % (BASE_DIR,
                                             config.FILES_PATH, service),
            '%s%sComunicacao/%s/response/' % (BASE_DIR,
                                              config.FILES_PATH, service),
        ]
...
def create_pem_files(certificado):
    ...
    pkcs12 = crypto.load_pkcs12(
        open(certificado.cert_host(), 'rb').read(),
        certificado.senha)
    ...
    if not os.path.isfile(BASE_DIR + certificado.cert_pem_file()):
        cert_str = crypto.dump_certificate(
            crypto.FILETYPE_PEM, pkcs12.get_certificate())
        open(certificado.cert_pem_file(), 'wb').write(cert_str)

    if not os.path.isfile(BASE_DIR + certificado.key_pem_file()):
        key_str = crypto.dump_privatekey(
            crypto.FILETYPE_PEM, pkcs12.get_privatekey())
        open(certificado.key_pem_file(), 'wb').write(key_str)
